...
def scan_national_id_with_gemini(image_file):
    """
    Sends ID image to Gemini Vision (free tier).
    ...
    if model is None:
        if not GEMINI_AVAILABLE:
            return {"error": "google-generativeai not installed — add it to requirements.txt"}
        return {"error": "GEMINI_API_KEY not set in Streamlit secrets"}
    try:
        img = PIL.Image.open(io.BytesIO(image_file.getvalue()))
...
def transcribe_audio_gemini(audio_bytes, language_hint="English"):
    """
    Transcribes audio using Gemini 1.5 Flash multimodal (free tier).
    Uploads audio via the File API then asks Gemini to transcribe it.
    ...
    try:
        import io as _io
        # Upload via File API — more reliable than inline base64 for audio
        audio_io = _io.BytesIO(audio_bytes)
        audio_io.name = "recording.webm"
        uploaded = genai.upload_file(audio_io, mime_type="audio/webm")
        resp = model.generate_content([
            uploaded,
        ...
        # Clean up uploaded file to stay within free tier storage
        try:
...
def process_voice_command_with_gemini(user_query, user_profile, gov_db, blueprint_db, masterclass_db):
    """
    Cross-references the user's query (from voice or text) against all three
    databases and returns a personalised response via Gemini (free tier).
    Automatically responds in Swahili or Arabic if the query is in that language.
    """
    model = _gemini_model()
    if model is None:
        if not GEMINI_AVAILABLE:
            return "google-generativeai is not installed. Add it to requirements.txt."
...
Your job: help this citizen find the most relevant grants, business blueprints, and financial advice based on their profile and question.
