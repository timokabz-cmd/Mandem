...
def transcribe_audio_gemini(audio_bytes, language_hint="English"):
    """
    Transcribes audio using Gemini multimodal (free tier).
    language_hint: 'English', 'Swahili', or 'Arabic'
    No Google Cloud account needed — just the Gemini API key.
    """
    model = _gemini_model()
    if model is None:
        return None
    try:
        ...
        resp = model.generate_content([
            ...
            f"This audio is spoken in {language_hint}. Transcribe exactly what is said, word for word."
        ])
