...
# ==================================================================
    ...
    st.caption("⚠️ Demo metrics below are illustrative only — replace once real WhatsApp/USSD traffic exists. The feedback log is real.")
    ...
    st.write("---")
    st.subheader("Geographic and Sectoral Demand")
    st.caption("⚠️ Demo data — illustrative only.")
    ...
    st.write("---")
    st.subheader("Live Feedback Log")
    st.caption("Real feedback-button presses from this running instance — tagged by district when the user has a profile.")
    if st.session_state.feedback_log:
        df_fb = pd.DataFrame(st.session_state.feedback_log)
        st.dataframe(df_fb, use_container_width=True)
        if "district" in df_fb.columns:
            st.subheader("Feedback by District")
            st.bar_chart(df_fb.groupby("district").size())
    else:
        st.info("No feedback yet. Use the feedback buttons in the WhatsApp Simulator to generate entries.")

    st.write("---")
    st.subheader("📄 Generate LC1 Weekly Digest PDF")
    st.caption("A printable one-page summary for posting at an LC1 office alongside the QR code. The full digital cards are available by scanning the QR.")
    ...
    if PDF_AVAILABLE:
        ...
        with digest_col2:
            n_gov = st.slider("Government service cards to include:", 1, min(5, len(st.session_state.gov_db)), 3)
            n_bp = st.slider("Business blueprints to include:", 1, min(3, len(st.session_state.blueprint_db)), 2)
            n_fl = st.slider("Financial literacy lessons to include:", 1, min(5, len(st.session_state.masterclass_db)), 3)

        if st.button("🖨️ Generate Weekly Digest PDF"):
            gov_subset = ([c for c in st.session_state.gov_db if c.get("stage") == digest_stage][:n_gov]
                          if digest_stage != "All Stages" else st.session_state.gov_db[:n_gov])
            bp_subset = st.session_state.blueprint_db[:n_bp]
            fl_subset = ([m for m in st.session_state.masterclass_db if m.get("stage") == digest_stage][:n_fl]
                         if digest_stage != "All Stages" else st.session_state.masterclass_db[:n_fl])
            pdf_bytes = generate_digest_pdf(gov_subset, bp_subset, fl_subset, digest_district)
            if pdf_bytes:
                st.download_button("📥 Download Digest PDF", data=pdf_bytes,
                                   file_name=f"edgelab_digest_{digest_district or 'all'}.pdf",
                                   mime="application/pdf")
    else:
        st.warning("PDF generation is unavailable — ensure fpdf2 is in requirements.txt and redeploy.")
